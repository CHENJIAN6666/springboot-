...
public class HMacMD5Util {
	public static Logger logger = LoggerFactory.getLogger(HMacMD5Util.class);
	 ...
	 * @throws NoSuchAlgorithmException
	 */
	public static String getHmacMd5(String key, String data) throws NoSuchAlgorithmException {
		String result = "";
		if (key == null || data == null || key.equals("")) {
			logger.error("密钥或数据为空Key:" + key + " 数据：" + data);
			return null;
		}

		byte[] byteResult = getHmacMd5Bytes(key.getBytes(), data.getBytes());
		result = HMacMD5Util.bytesToHexString(byteResult);
		return result;
	}
